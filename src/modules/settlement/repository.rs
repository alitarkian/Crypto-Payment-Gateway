...
pub trait SettlementRepository: Send + Sync {
    ...
    async fn find_settlement_by_payment_id(&self, payment_id: Uuid) -> Result<Option<Settlement>, SettlementError>;
    async fn find_pending_settlements(&self) -> Result<Vec<Settlement>, SettlementError>;
    async fn find_settlements_by_merchant(&self, merchant_id: Uuid) -> Result<Vec<Settlement>, SettlementError>;
    ...
    async fn find_open_batch_for_merchant(&self, merchant_id: Uuid) -> Result<Option<SettlementBatch>, SettlementError>;
    async fn find_batches_by_merchant(&self, merchant_id: Uuid) -> Result<Vec<SettlementBatch>, SettlementError>;
}